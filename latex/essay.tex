...
Therefore, vertices far away on the graph that correspond to unfamiliar
...
\foreach \m/\l [count=\y] in {1,2,3,missing,4}
  \node [every neuron/.try, neuron \m/.try] (input-\m) at (0,2.25-\y*0.75) {};
...
\foreach \l [count=\i] in {1,d}
  \node [below] at (hidden-\i.north) {$h_\l$};

\foreach \l [count=\i] in {1,2,3,|V|}
  \draw [->] (output-\i) -- ++(1,0)
    node [above, midway] {$\P(c_{\l} | v)$};
...
\foreach \i in {1,...,2}
  \foreach \j in {1,2,3,...,4}
    \draw[gray] [->] (hidden-\i) -- (output-\j);

\node [align=center, above] at (0,2) {Input Vector\\\textit{One-Hot Encoding}};
...
\caption{The structure of the neural network that is trained to obtain
  the weight matrix $W$ used for the network embeddings in the SkipGram algorithm. The weight matrix $W \in \R^{|V| \times d}$ is represented by the orange lines
  between the input layer and the hidden layer which has $d$ neurons.}
\end{figure}\\
...
text-associated DeepWalk (2015, \cite{yang2015}) which incorporates text features
...
\begin{proof}
  ...
  \[\frac{\#(v, c)_{\rar}}{|\D_{\rar}|} = \frac{1}{l-r+1}\sum_{j = 0}^{l-r} Y_j \overset{p}{\longrightarrow} \pi_vQ_{vc} \]
  ...
  Since this holds for a random walk initialised at any starting distribution, it holds for any number of random walks initialized from the vertices in any order since the corresponding value of $\frac{\#(v, c)_{\rar}}{|\D_{\rar}|}$ is just this result averaged over the number of random walks and the distribution used to pick their initial vertices.
  \end{proof}
We have one more lemma to prove before we get to the main theorem of this chapter.
\begin{lemma}
  As $l \to \infty$, we have
...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{NaïveUpdate}{$\G^{t+1}, \W^t, \V^{t+1}_{\text{affected}}, \gamma, l$}
    \LineComment{Initialise $\gamma$ new walks from each of the affected vertices in $\V^{t+1}_{\text{affected}}$}
    \State $\W$ \leftarrow initWalks($\V^{t+1}_{\text{affected}}, \gamma$)
    \State $\W^{t+1}$ \leftarrow randomwalk($\W, l, \G^{t+1}$)
    \LineComment{Replace the old walks and add the new ones}
