...
this vertex onwards since it evolves with incorrect transition probabilities.
Therefore only the affected walks need to be updates to retain a statistically
...
One option for this is to simply recalculate
all of the vertex representations, however this would be very inneficient since
it is possible to use the previous representations, since the new
representations will be very similar if only a small portion of the graph
structure has been changed. 



The idea behind this is similar to using pretrained weights in a transfer
learning problem instead of initialising weights randomly.
