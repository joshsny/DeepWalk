...
\begin{lemma}
  Let $N$ be a random variable such that $\frac{N}{n} \overset{p}{\longrightarrow} \mu$ as $n \to \infty$ and let $(Y_k)_{k \geq 1}$ be i.i.d random variables with $\E[Y_1] < \infty$. Then
  \[\frac{1}{n}\sum_{k=1}^N Y_k \overset{p}{\longrightarrow} \mu\E[Y_1]\]
\end{lemma}
[Notes: Will probably push the proof of this to the appendix. Note that all Lemma/Theorem numbers need to be checked and changed with the introduction of this extra one.]
\begin{proof}
Firstly we will show that it converges in expectation appropriately
\begin{align*}
...
&= \E[Y_1]\sum_{R \geq 0} \frac{R}{n}\P(N = R) = 
\end{align*}
\end{proof}
...
follows since $P$ is irreducible, and thus $\pi$ is the unique invariant distribution of $Z$ by Theorem 2.4. Let $N_v(l)$ denote the number of visits of $Z$ to $v$ by time $l$\footnote{Here $l$ has been rescaled by a factor of $\frac{1}{r}$ to match $Z$}.
...
Now let $i_1, i_2, \dots$ be the times of successive visits of $Z$ to $v$ and $Y_k = \mathbbm{1}(Z_{i_k+1} = c)$. Since $(Y_k)_{k \geq 1}$ are i.i.d, we can apply [Lemma X], yielding the first limit:
