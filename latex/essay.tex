...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \LineComment{Initialise new walks from affected vertices}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \LineComment Trim the affected walks
    \State $W_e$ \leftarrow trim($W_{\text{affected}}, \V_e$)
    \LineComment \leftarrow Initialise walks of length r from the new vertices
    \State $W_n$ \leftarrow initWalks($\V_n, r$)
    \LineComment Take the union of the new walks to create the update corpus
