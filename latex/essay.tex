...
\end{align}
and so the objective function $l$ can be expressed as
\begin{equation}
 l =  \sum_{v \in V}\sum_{c \in V}\#(v, c)\log{\sigm(\vec{v} \cdot \vec{c})} + \sum_{v \in V}\#(v)\left(b \cdot \E_{\tilde{c} \sim P_{\D}}[\log{\sigma(-\vec{v} \cdot \vec{\tilde{c}})}] \right)
 \end{equation}
...
the pointwise mutual information (PMI) shifted by a factor of $\log k$. PMI was
