...
\begin{proof}
  ...
  \end{align*}
  where the second inequality comes from applying the Cauchy-Schwarz inequality together with the bounded variance for the first term and using $n > N$ for the second. For any $\epsilon > 0$, we can pick $\delta$ sufficiently small and $N_1$ such that for all $n > N_1 \gg N(\delta)$ we have $Var\left(\frac{S_n}{n}\right) < \epsilon$ and so $Var\left(\frac{S_n}{n}\right) \to 0$.\\
...
\begin{proof}
  ...
  Since the $Y_j$ are indicator functions their variance is bounded above by 1. Applying Proposition 2.5 gives
