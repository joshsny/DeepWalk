...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{DeepWalk}{$G, w, d, \gamma, l$}
    \Input{network $G(V, E)$}
    \EndProcedure
...
within distance $k$ where $k$ is the size of the window. SkipGram
minimises the following objective function

\[L = - \sum_{c \in C(w)} \log{\P(c | w)}\]
...
nodes within a window of size $k$, from which target-context pairs $(v, c)$ are
constructed and added to a corpus $\D$. After these context nodes have been
...
[Algorithm XX] shows how DeepWalk is applied. $\gamma$ represents the number of
random walks that are started from each node and at the start of each pass the
nodes are shuffled so that they are traversed in a random order. For each node
$v \in V$ a random walk $\mathcal{W_v}$ of length $l$ is generated and used to
update the network embeddings using the SkipGram algorithm.\\
*** Why is it important that the vertices are traversed in a random order, given
we are just building a corpus? ***\\
In the context of graph networks, SkipGram trains a neural network to do the
following task:\\
Given an input vertex $v$ and a random walk, $W_{v}$, of length $2w+1$ with $v$
